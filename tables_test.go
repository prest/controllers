...
func TestSelectFromTables(t *testing.T) {
	...
	var testCases = []struct {
		...
		{"execute select in a table with select fields", "/prest/public/test5?_select=celphone,name", "GET", http.StatusOK, "[{\"celphone\":\"444444\",\"name\":\"prest tester\"}]"},
...
func TestUpdateFromTable(t *testing.T) {
	...
	var testCases = []struct {
		...
		{"execute update in a table with where clause", "/prest/public/test?name=$eq.nuveo&_returning=name", m, http.StatusOK, "[{\"name\": \"prest\"}]"},
